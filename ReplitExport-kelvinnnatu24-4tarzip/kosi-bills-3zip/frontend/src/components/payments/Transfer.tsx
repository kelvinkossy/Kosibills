...
import { Send, Search, User as UserIcon, Lock, Loader2, CheckCircle2, ArrowLeft, AlertCircle, ShieldCheck } from 'lucide-react';
...
type Step = 'find' | 'amount' | 'pin' | 'processing' | 'success';
...
export default function Transfer({ user, setView, setUser }: TransferProps) {
  ...
  const handleSearch = async () => {
    ...
    setIsLoading(true);
    try {
      const r = await fetch(`/api/users/find?phone=${encodeURIComponent(phone.trim())}`);
  ...
  const stepIndex = { find: 0, amount: 1, pin: 2, processing: 2, success: 3 }[step];
  ...
  return (
    <div className="max-w-md mx-auto space-y-4 pb-24">
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => {
          if (step === 'find') setView('dashboard');
          else if (step === 'amount') setStep('find');
          else if (step === 'pin') setStep('amount');
          else if (step === 'processing') {} // no back during processing
          else setStep('find');
        }}
        ...
        <div className="flex gap-2">
          {[0, 1, 2].map((i) => (
            <div key={i} className={`h-1 flex-1 rounded-full transition-all duration-300 ${
              stepIndex >= i ? 'bg-emerald-500' : 'bg-slate-200 dark:bg-slate-800'
            }`} />
          ))}
        </div>
      )}
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'pin' && foundUser && (
          <motion.div key="pin" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
            ...
            <button onClick={handleTransfer} disabled={pin.length !== 4}
              className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white font-black rounded-2xl transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20">
              <Send className="w-5 h-5" /> Send ₦{Number(amount).toLocaleString()}
            </button>
          </motion.div>
        )}

        {/* Step 4: Processing */}
        {step === 'processing' && foundUser && (
          <motion.div key="processing" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
            className="bg-white dark:bg-slate-900 rounded-[1.5rem] p-10 border border-slate-100 dark:border-slate-800 shadow-sm text-center space-y-6">
            ...
            <div>
              <h2 className="font-black text-slate-800 dark:text-white text-xl">Processing Transfer</h2>
              <motion.p
                key={processingMsg}
            ...
            <div className="bg-emerald-50 dark:bg-emerald-950/30 rounded-2xl p-3 flex gap-2 text-left">
              <AlertCircle className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
              <p className="text-xs text-emerald-700 dark:text-emerald-300 font-medium">Please keep this screen open. Do not press back.</p>
            </div>
          </motion.div>
        )}
