...
import { Eye, EyeOff, RefreshCw, Copy, ArrowUpRight, ArrowDownLeft, Send, Plus, TrendingUp, TrendingDown } from 'lucide-react';
...
export default function WalletCard({
  ...
  isLoading, setShowBankDetails, setShowFundModal, isFunding, setView, todayChange = 0
}: WalletCardProps) {
  ...
  const hasTodayChange = todayChange !== 0;
      ...
      >
        ...
        <div className="relative z-10">
          ...
          {hasTodayChange && !hideBalance && (
            ...
            >
              <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full ${isPositive ? 'bg-emerald-500/20' : 'bg-red-500/20'}`}>
                {isPositive
                  ? <TrendingUp className="w-3 h-3 text-emerald-400" />
                  : <TrendingDown className="w-3 h-3 text-red-400" />}
                <span className={`text-[11px] font-black ${isPositive ? 'text-emerald-400' : 'text-red-400'}`}>
                  {isPositive ? '+' : '-'}₦{absChange.toLocaleString('en-NG', { minimumFractionDigits: 2 })} today
                </span>
              </div>
            </motion.div>
          )}

          {!hasTodayChange && <div className="mb-5" />}
