...
export default function Dashboard({ setView, user, setUser }: DashboardProps) {
  ...
  const { thisMonthSpent, lastMonthSpent, topExpenseCategory, budgetUsedPercent, aiTip, upcomingBillNudge, todayChange } = useMemo(() => {
    ...
    const todayChange = todayTxs.reduce((acc, tx) => acc + tx.amount, 0);
