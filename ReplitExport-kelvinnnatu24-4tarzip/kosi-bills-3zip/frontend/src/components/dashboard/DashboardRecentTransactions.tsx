...
const RecentTransactions = memo(function RecentTransactions({ transactions, isLoading, setView }: RecentTransactionsProps) {
  const groups = groupByDate(transactions.slice(0, 5));
          ...
          groups.map((group, gi) => (
            <div key={group.label}>
              <div className="px-4 pt-3 pb-1.5 flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">{group.label}</span>
              ...
              {group.items.map((tx, i) => {
                ...
                const globalIndex = gi * 5 + i;
                return (
                  <motion.div key={tx.id}
                    initial={{ opacity: 0, x: -8 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: globalIndex * 0.04 }}
                    className="flex items-center justify-between px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors border-b border-slate-50 dark:border-slate-800/50 last:border-0">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${meta.bg}`}>
                        <Icon className={`w-5 h-5 ${meta.color}`} />
                      </div>
                      <div>
                        <p className="font-bold text-slate-800 dark:text-slate-200 text-sm truncate max-w-[160px]">{tx.description}</p>
