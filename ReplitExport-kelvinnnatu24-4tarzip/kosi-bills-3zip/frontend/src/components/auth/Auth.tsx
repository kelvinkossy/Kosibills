...
export default function Auth({ onLogin, initialMode = 'login' }: AuthProps) {
  ...
  const setError = (msg: string) => { setErrorMsg(msg); toast.error(msg); setShakeKey(k => k + 1); };
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {!isForgotMode && (
                  <div className="flex items-center justify-center gap-1.5 mt-3">
                    ...
                    <p className="text-[11px] text-slate-500 font-medium">256-bit encryption · SSL secured</p>
                  </div>
                )}
              </form>
              </motion.div>
