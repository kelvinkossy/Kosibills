...
// Phone frame wrapper — full screen on mobile, centered frame on desktop
const PhoneFrame = ({ children }: { children: ReactNode }) => (
  <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-950/40 to-slate-900 flex items-center justify-center font-sans">
    <div className="relative w-full md:w-[430px] h-screen md:h-[900px] bg-slate-50 dark:bg-[#0a0f1e] md:rounded-[2.5rem] md:shadow-[0_40px_120px_rgba(0,0,0,0.6)] md:border md:border-slate-700/50 overflow-hidden flex flex-col">
      {children}
      <Toaster position="top-center" toastOptions={{ className: 'font-bold rounded-2xl shadow-xl text-sm', duration: 4000 }} />
    </div>
  </div>
);
...
function AppContent() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-emerald-950/40 to-slate-900 flex items-center justify-center font-sans">
      {/* Phone frame wrapper — full screen on mobile, centered frame on desktop */}
      <div className="relative w-full md:w-[430px] h-screen md:h-[900px] flex flex-col bg-slate-50 dark:bg-[#0a0f1e] md:rounded-[2.5rem] md:shadow-[0_40px_120px_rgba(0,0,0,0.6)] md:border md:border-slate-700/50 text-slate-900 dark:text-slate-100 transition-colors duration-300 overflow-hidden">

        {/* Status bar spacer (mobile safe area) */}
        <div className="h-[env(safe-area-inset-top,0px)] bg-white/90 dark:bg-slate-900/90" />

        {/* ── App Header ── */}
        <header className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl border-b border-slate-200/60 dark:border-slate-800/60 z-30 flex-shrink-0">
          <div className="flex items-center justify-between px-4 h-14 relative">
            ...
            <div className="w-10">
              {!isMainTab ? (
                <button
                  ...
                  className="p-2 -ml-2 text-slate-600 dark:text-slate-300 active:scale-90 transition-transform rounded-xl"
                  aria-label="Back"
                >
            ...
            <div className="absolute left-1/2 -translate-x-1/2 flex items-center gap-2">
              <AnimatePresence mode="wait">
                <motion.h1
                  ...
                  className="text-[15px] font-black text-slate-800 dark:text-slate-100 tracking-tight"
                >
                  {viewTitle}
                </motion.h1>
              </AnimatePresence>
              {user?.isLiveMode && (
                <span className="bg-red-500 text-white text-[9px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-full flex items-center gap-0.5">
            ...
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 active:scale-90 transition-all rounded-xl"
              ...
              <div className="relative">
                <button
                  ...
                  className="relative p-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 active:scale-90 transition-all rounded-xl"
                      ...
                      >
                        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-2">
                            {notifications.length > 0 && (
                              <button onClick={deleteAllNotifications} className="text-slate-400 hover:text-red-500 transition-colors">
                              ...
                              {notifications.map(notification => (
                                ...
                                >
                                  <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 ${
                                    ...
                                    {notification.type === 'alert' ? <ShieldCheck className="w-4 h-4" /> :
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-1.5">
                                      <p className={`text-xs font-bold truncate ${!notification.read ? 'text-slate-800 dark:text-slate-100' : 'text-slate-500 dark:text-slate-400'}`}>
                                        {notification.title}
                                      </p>
                                      {!notification.read && <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 flex-shrink-0" />}
                                    </div>
                                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate">{notification.message}</p>
                                    <p className="text-[10px] text-slate-400 mt-1">{new Date(notification.date).toLocaleDateString()}</p>
                                  </div>
                                </div>
                              ))}
                            </div>
                        ...
                        {unreadCount > 0 && (
                          <div className="p-3 border-t border-slate-100 dark:border-slate-800 text-center">
                            <button onClick={markAllAsRead} className="text-xs font-bold text-emerald-600 dark:text-emerald-400">Mark all as read</button>
                          </div>
                        )}
                      </motion.div>
                    </>
              ...
              <button
                ...
                className="ml-1 w-8 h-8 bg-emerald-100 dark:bg-emerald-900/40 rounded-full flex items-center justify-center text-emerald-700 dark:text-emerald-400 relative overflow-hidden active:scale-90 transition-transform flex-shrink-0"
              >
                {user?.profilePhoto ? (
                ...
                <div className="absolute -bottom-0.5 -right-0.5 bg-white dark:bg-slate-900 rounded-full p-[2px]">
        ...
        {/* ── Page Content ── */}
        <main className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom, 0px) + 4rem)' }}>
          <AnimatePresence mode="wait">
            <motion.div
              key={currentView}
        ...
        {/* ── Bottom Navigation ── */}
        <nav
          className="flex-shrink-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-t border-slate-200/60 dark:border-slate-800/60 z-40"
          style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}
        >
          <div className="flex items-stretch h-16">
            {bottomNav.map((item) => {
              const Icon = item.icon;
              const isActive = currentView === item.view || (item.view === homeView && currentView === homeView);
              return (
                <button
                  key={item.name}
                  onClick={() => navigateTo(item.view)}
                  className="flex-1 flex flex-col items-center justify-center gap-1 relative active:scale-90 transition-transform"
                ...
                >
                  <AnimatePresence>
                    {isActive && (
                      <motion.div
                        ...
                        className="absolute inset-x-3 inset-y-2 bg-emerald-500/10 dark:bg-emerald-400/10 rounded-xl"
                  ...
                  >
                    <Icon
                      className={`w-5 h-5 transition-colors duration-200 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-600'}`}
                      strokeWidth={isActive ? 2.5 : 1.8}
                    />
                  </motion.div>
                  <span className={`text-[10px] font-bold transition-colors duration-200 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400 dark:text-slate-600'}`}>
                    {item.name}
                  </span>
                </button>
              );
        ...
        <Toaster
          position="top-center"
          toastOptions={{
            className: 'dark:bg-slate-800 dark:text-slate-100 font-bold rounded-2xl shadow-xl border border-slate-100 dark:border-slate-700 text-sm',
            duration: 4000,
          }}
        />
      </div>
