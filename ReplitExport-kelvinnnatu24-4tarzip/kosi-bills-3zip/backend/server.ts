...
async function startServer() {
  ...
  // CORS — allow the frontend dev server, Replit preview domains, and production frontend
  ...
  const isAllowedOrigin = (origin: string | undefined): boolean => {
    if (!origin) return true; // server-to-server (Vite proxy)
    if (allowedOrigins.includes(origin)) return true;
    // Allow any Replit dev/preview subdomain
    if (/^https?:\/\/[a-z0-9-]+(\.[\w-]+)*\.(replit\.dev|repl\.co|replit\.app)$/.test(origin)) return true;
    return false;
  };
  ...
  app.post('/api/auth/login', authLimiter, async (req, res) => {
    ...
    // Always run bcrypt to prevent timing attacks that reveal email existence
  ...
  app.post('/api/sub-wallets/create', authenticateToken, (req: any, res) => {
    ...
    // Ensure the authenticated user can only create wallets for themselves
    if (Number(ownerId) !== Number(req.user.id)) return res.status(403).json({ error: 'Forbidden' });
    if (!name || typeof name !== 'string' || name.trim().length < 1) return res.status(400).json({ error: 'Wallet name is required' });
    try {
      const stmt = db.prepare('INSERT INTO sub_wallets (owner_id, name, balance) VALUES (?, ?, 0)');
      const info = stmt.run(ownerId, name.trim());
  ...
  app.post('/api/sub-wallets/fund', authenticateToken, paymentLimiter, (req: any, res) => {
    const { userId, subWalletId, amount } = req.body;
    // Ensure the authenticated user can only fund their own wallets
    if (Number(userId) !== Number(req.user.id)) return res.status(403).json({ error: 'Forbidden' });
    ...
    try {
      const transaction = db.transaction(() => {
        ...
        const user = db.prepare('SELECT balance, account_status FROM users WHERE id = ?').get(userId) as any;
        if (!user) throw new Error('User not found');
        if (user.account_status !== 'active') throw new Error('Account is frozen');
        if (user.balance < parsedAmount) throw new Error('Insufficient balance');

        // Verify the sub-wallet belongs to this user
        const wallet = db.prepare('SELECT id FROM sub_wallets WHERE id = ? AND owner_id = ?').get(subWalletId, userId) as any;
        if (!wallet) throw new Error('Sub-wallet not found');

        db.prepare('UPDATE users SET balance = balance - ? WHERE id = ?').run(parsedAmount, userId);
        db.prepare('UPDATE sub_wallets SET balance = balance + ? WHERE id = ?').run(parsedAmount, subWalletId);
        const newBalance = user.balance - parsedAmount;
        db.prepare('INSERT INTO transactions (user_id, type, description, amount, date, status, category, sub_wallet_id, balance_after) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
          .run(userId, 'Sub-Wallet Funding', `Funded sub-wallet #${subWalletId}`, -parsedAmount, new Date().toISOString(), 'success', 'Transfer', subWalletId, newBalance);
      });

      transaction();
      createNotification(userId, 'Sub-Wallet Funded', `You successfully funded sub-wallet #${subWalletId} with ₦${parsedAmount.toLocaleString()}`, 'success');
      res.json({ success: true });
    } catch (error: any) {
      if (['Insufficient balance', 'Account is frozen', 'Sub-wallet not found', 'User not found'].includes(error.message)) {
  ...
  app.post('/api/sub-wallets/add-member', authenticateToken, (req: any, res) => {
    ...
    try {
      // Verify the authenticated user owns this sub-wallet
      const wallet = db.prepare('SELECT id FROM sub_wallets WHERE id = ? AND owner_id = ?').get(subWalletId, req.user.id) as any;
      if (!wallet) return res.status(403).json({ error: 'Forbidden: you do not own this sub-wallet' });
      ...
      if (user.id === req.user.id) return res.status(400).json({ error: 'Cannot add yourself as a member' });
  ...
  app.post('/api/payments/pay', authenticateToken, paymentLimiter, (req: any, res) => {
    ...
    try {
      ...
      // Pre-check PIN before entering transaction (PIN is immutable during request)
      const userForPin = db.prepare('SELECT pin, is_agent FROM users WHERE id = ?').get(userId) as any;
      if (!userForPin) return res.status(404).json({ success: false, error: 'User not found' });

      if (userForPin.pin && (!pin || !verifyPin(pin, userForPin.pin))) {
      ...
      if (userForPin.is_agent) {
        finalAmount = parsedAmount * 0.98; // 2% agent discount
      }

      // All balance work happens inside a single atomic transaction to prevent double-spend
      const transaction = db.transaction(() => {
        // Re-read balance inside the transaction for race-condition safety
        const fresh = db.prepare('SELECT balance, account_status FROM users WHERE id = ?').get(userId) as any;
        if (!fresh) throw new Error('User not found');
        if (fresh.account_status !== 'active') throw new Error('Account is frozen');
        if (fresh.balance < finalAmount) {
          // Log failed attempt
          db.prepare('INSERT INTO transactions (user_id, type, description, amount, date, status, balance_after, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
            .run(userId, type, description, -finalAmount, new Date().toISOString(), 'failed', fresh.balance, metadata ? JSON.stringify(metadata) : null);
          throw new Error('Insufficient balance');
        }
        db.prepare('UPDATE users SET balance = balance - ? WHERE id = ?').run(finalAmount, userId);
        const newBalance = fresh.balance - finalAmount;
        const result = db.prepare('INSERT INTO transactions (user_id, type, description, amount, date, status, balance_after, metadata) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
          .run(userId, type, description, -finalAmount, new Date().toISOString(), 'success', newBalance, metadata ? JSON.stringify(metadata) : null);
